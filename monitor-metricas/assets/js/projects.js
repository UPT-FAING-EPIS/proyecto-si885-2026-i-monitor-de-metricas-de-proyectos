...
const uniq = (arr) => Array.from(new Set(arr)).sort((a, b) => a.localeCompare(b, 'es'));
...
const init = () => {
    ...
    const paginationLabel = qs('#paginationLabel');
    const prevPage = qs('#prevPage');
    const nextPage = qs('#nextPage');
    const pageButtons = qs('#pageButtons');
    if (!search || !status || !owner || !dateFrom || !dateTo || !clearFilters || !cards || !resultCount || !paginationLabel || !prevPage || !nextPage || !pageButtons)
        return;
    ...
    const wireCardActions = () => {
        cards.querySelectorAll('button[data-action][data-id]').forEach((btn) => {
            btn.addEventListener('click', () => {
