...
type Payload = { projects: SelectItem[]; teams: SelectItem[] };

type ChartSeries = { label: string; values: number[]; color: 'pm' | 'sky' | 'emerald' | 'rose' | 'slate'; dashed?: boolean; fill?: boolean };
...
const readPayload = (): Payload => {
  const el = qs<HTMLScriptElement>('#pmAnalyticsData');
  if (!el?.textContent) return { projects: [], teams: [] };
  try {
...
const daysBetween = (from: number, to: number) => Math.max(1, Math.round((to - from) / (24 * 60 * 60 * 1000)) + 1);
...
const seriesSmooth = (values: number[], amount: number) => {
  const out = [...values];
...
const palette = (key: ChartSeries['color']) => {
  const dark = document.documentElement.classList.contains('dark');
  const map: Record<ChartSeries['color'], { stroke: string; fill: string; muted: string }> = {
...
const renderLine = (svg: SVGSVGElement, series: ChartSeries[], height = 240) => {
  const w = 600;
  const h = height;
  const padX = 18;
  const padY = 20;
...
const renderBars = (svg: SVGSVGElement, valuesA: number[], valuesB: number[] | null, colorA: ChartSeries['color'], colorB: ChartSeries['color']) => {
  const w = 600;
  const h = 240;
  const pad = 18;
  ...
  const gap = valuesB ? groupW * 0.10 : groupW * 0.15;
...
const init = () => {
  ...
  const projectSelect = qs<HTMLSelectElement>('#projectSelect');
  const teamSelect = qs<HTMLSelectElement>('#teamSelect');
  const dateFrom = qs<HTMLInputElement>('#dateFrom');
  const dateTo = qs<HTMLInputElement>('#dateTo');
  const compareToggle = qs<HTMLInputElement>('#compareToggle');
  const summaryPill = qs<HTMLElement>('#summaryPill');

  const exportBtn = qs<HTMLButtonElement>('#exportBtn');
  const exportMenu = qs<HTMLDivElement>('#exportMenu');
  const exportCsv = qs<HTMLButtonElement>('#exportCsv');
  const exportJson = qs<HTMLButtonElement>('#exportJson');
  const resetFilters = qs<HTMLButtonElement>('#resetFilters');

  const chartBurnDown = qs<SVGSVGElement>('#chartBurnDown');
  const chartBurnUp = qs<SVGSVGElement>('#chartBurnUp');
  const chartProductivity = qs<SVGSVGElement>('#chartProductivity');
  const chartVelocity = qs<SVGSVGElement>('#chartVelocity');
  const chartLeadTime = qs<SVGSVGElement>('#chartLeadTime');
  const chartCycleTime = qs<SVGSVGElement>('#chartCycleTime');

  const productivityBadge = qs<HTMLElement>('#productivityBadge');
  const velocityBadge = qs<HTMLElement>('#velocityBadge');
  const leadBadge = qs<HTMLElement>('#leadBadge');
  const cycleBadge = qs<HTMLElement>('#cycleBadge');
  ...
  const toggleExportMenu = (open: boolean) => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!t) return;
    if (exportBtn.contains(t) || exportMenu.contains(t)) return;
  ...
  const generate = (seedKey: string, count: number, profile: 'burndown' | 'burnup' | 'productivity' | 'velocity' | 'lead' | 'cycle') => {
    ...
    if (profile === 'productivity') {
      for (let i = 0; i < count; i++) values.push(8 + r() * 12 + Math.sin(i / 4) * 2);
      return seriesSmooth(values, 0.45).map((v) => Math.max(0, v));
    }
    if (profile === 'velocity') {
      for (let i = 0; i < count; i++) values.push(24 + r() * 18 + Math.sin(i / 2) * 3);
      return seriesSmooth(values, 0.25).map((v) => Math.max(0, v));
    }
    if (profile === 'lead') {
      for (let i = 0; i < count; i++) values.push(9 + r() * 10 + Math.sin(i / 3) * 1.8);
      return seriesSmooth(values, 0.35).map((v) => Math.max(1, v));
    }
    for (let i = 0; i < count; i++) values.push(4 + r() * 8 + Math.sin(i / 3) * 1.2);
  ...
  const render = () => {
    ...
    const deltaPct = (cur: number, prev: number | null) => (prev === null || prev === 0 ? null : ((cur - prev) / prev) * 100);
