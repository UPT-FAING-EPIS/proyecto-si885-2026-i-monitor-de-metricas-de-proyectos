...
type KPI = { label: string; value: number; delta: number; tone: 'pm' | 'emerald' | 'rose' | 'amber' };
type Report = { id: string; name: string; desc: string; kpis: KPI[] };
type Payload = { reports: Report[]; filters: { projects: string[]; teams: string[]; periods: string[] } };
...
const readPayload = (): Payload => {
  const el = qs<HTMLScriptElement>('#pmPowerBIData');
  if (!el?.textContent) return { reports: [], filters: { projects: [], teams: [], periods: [] } };
  try {
...
const toneClasses = (tone: KPI['tone']) => {
  if (tone === 'emerald') return 'bg-emerald-50 text-emerald-700 ring-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-200 dark:ring-emerald-500/20';
  if (tone === 'rose') return 'bg-rose-50 text-rose-700 ring-rose-100 dark:bg-rose-500/10 dark:text-rose-200 dark:ring-rose-500/20';
  if (tone === 'amber') return 'bg-amber-50 text-amber-800 ring-amber-100 dark:bg-amber-500/10 dark:text-amber-200 dark:ring-amber-500/20';
...
const buildEmbedHTML = (report: Report, filters: { project: string; team: string; period: string }, theme: Theme) => {
...
const init = () => {
  ...
  const reportSelect = qs<HTMLSelectElement>('#reportSelect');
  const reportTitle = qs<HTMLElement>('#reportTitle');
  const reportDesc = qs<HTMLElement>('#reportDesc');
  const embedMeta = qs<HTMLElement>('#embedMeta');
  const kpiRow = qs<HTMLDivElement>('#kpiRow');
  const frame = qs<HTMLIFrameElement>('#reportFrame');
  const embedContainer = qs<HTMLDivElement>('#embedContainer');

  const filterProject = qs<HTMLSelectElement>('#filterProject');
  const filterTeam = qs<HTMLSelectElement>('#filterTeam');
  const filterPeriod = qs<HTMLSelectElement>('#filterPeriod');
  const filtersSummary = qs<HTMLElement>('#filtersSummary');
  const applyFilters = qs<HTMLButtonElement>('#applyFilters');
  const resetFilters = qs<HTMLButtonElement>('#resetFilters');

  const fullscreenBtn = qs<HTMLButtonElement>('#fullscreenBtn');
  const exportPdf = qs<HTMLButtonElement>('#exportPdf');
  const exportXlsx = qs<HTMLButtonElement>('#exportXlsx');
  ...
  const state: { reportId: string; project: string; team: string; period: string } = saved
    ? (() => {
        try {
          return JSON.parse(saved) as { reportId: string; project: string; team: string; period: string };
        } catch {
  ...
  const renderKPIs = (report: Report) => {
  ...
  const persist = () => {
    localStorage.setItem(
      storageKey,
      JSON.stringify({ reportId: reportSelect.value, project: filterProject.value, team: filterTeam.value, period: filterPeriod.value })
    );
  };
